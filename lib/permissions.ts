 ...
 * GPS Watch Tracker - Sistema di controllo accessi
 */
...
/**
 * Verifica se l'utente può configurare dispositivi
...
/**
 * Verifica se l'utente può accedere alla control room (monitoraggio multi-utente)
 * Richiesto: assistente_control (livello 3) o superiore
 */
export function canAccessControlRoom(user: UserWithRuolo): boolean {
  return hasPermission(user.ruolo.livello, LIVELLI_ACCESSO.ASSISTENTE_CONTROL)
}

/**
 * Verifica se l'utente può monitorare altri utenti (parenti/tutelati)
 * Richiesto: controllo_parentale (livello 2) o superiore
 */
export function canMonitorOthers(user: UserWithRuolo): boolean {
...
/**
 * Verifica se l'utente può gestire orologi GPS
 * Richiesto: animatore_digitale (livello 4) o superiore
 */
export function canManageGPSWatches(user: UserWithRuolo): boolean {
  return hasPermission(user.ruolo.livello, LIVELLI_ACCESSO.ANIMATORE_DIGITALE)
}
...
export function getPermissionDescription(level: number): string {
  switch (level) {
    ...
    case LIVELLI_ACCESSO.ANIMATORE_DIGITALE:
      return 'Configurazione dispositivi e orologi'
    case LIVELLI_ACCESSO.ASSISTENTE_CONTROL:
      return 'Monitoraggio multi-utente in control room'
    case LIVELLI_ACCESSO.CONTROLLO_PARENTALE:
      return 'Monitoraggio parenti e tutelati'
